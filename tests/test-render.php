...
class Test_Render extends WP_UnitTestCase {
	...
	public function test_source_filter_can_provide_links() {
		add_filter(
			'blockroll_sources',
			function ( $sources ) {
		...
		$this->assertStringContainsString( 'https://source.example/feed/', $html );
		$this->assertStringNotContainsString( 'Manual Link', $html );
	}

	public function test_unknown_source_falls_back_to_manual_links() {
		$html = $this->render_block_html(
			array(
		...
		$this->assertStringContainsString( 'Manual Link', $html );
	}
}
